        ...
        try:
            ...
            if git_tmp and git_tmp.exists():
                try:
                    ...
                    try:
                        # dirs_exist_ok — يتجاوز مشكلة WinError 183
                        shutil.copytree(str(git_tmp), str(git_dir), dirs_exist_ok=True)
                        log("📦 .git أُعيد للفلاشة (نسخ)")
                    except Exception as _cp_err:
                        log(f"⚠️ فشل إعادة .git: {_cp_err}")
                finally:
                    try:
                        shutil.rmtree(git_tmp.parent, ignore_errors=True)
                    except Exception:
                        pass
