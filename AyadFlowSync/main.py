...
# -*- coding: utf-8 -*-
"""
AyadFlowSync v4.1 — نقطة الدخول (PyQt6)
"""
...
def main():
    ...
    try:
        ...
        logging.getLogger('AyadFlowSync').info("AyadFlowSync v4.1 starting (PyQt6)...")
    ...
    try:
        ...
        # ⚡ v4.1: حفظ crash log بجانب الـ EXE — مهم لأن console مخفي
        try:
            crash_file = Path(sys.executable).parent / "crash.log" \
                if getattr(sys, 'frozen', False) else Path("crash.log")
            crash_file.write_text(
                f"AyadFlowSync Crash — {__import__('datetime').datetime.now()}\n"
